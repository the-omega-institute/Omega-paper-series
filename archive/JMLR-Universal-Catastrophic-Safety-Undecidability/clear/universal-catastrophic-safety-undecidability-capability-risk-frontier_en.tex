...
\textbf{Third (Systematic Empirical Validation)}, we conduct extensive experiments across multiple datasets (CIFAR-10, CIFAR-100), training methods (standard ERM, PGD adversarial training, spectral normalization), and Lipschitz estimation techniques. Data-dependent Lipschitz estimators achieve $10^5{\times}$ tightness improvement over global bounds, validating Theorem~\ref{thm:data_dependent_bound}. On a complex 16${\times}$16 gridworld, we implement the SSR (Scope--Shield--Risk) framework, demonstrating a fundamental tradeoff: SSR completely eliminates training-phase catastrophes (13\%$\to$0\%) but induces extreme conservatism that sacrifices task completion in this environment, illustrating the inherent capability--safety tension predicted by our theory.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{Undecidability Results and Decidability Landscape}: We formalize the catastrophic safety verification problem \textsc{Unsafe} and prove that for computable policies and regular bad-prefix specifications, deciding whether $\Pr[\Bad(A,E,B)] > \varepsilon$ is $\Sigma_1^0$-complete (Theorems~\ref{thm:undecidability}, \ref{thm:completeness}). Consequently, no complete algorithmic verification procedure exists for general policies. We further establish a systematic decidability landscape: finite-state controllers yield PSPACE-decidable verification (Theorem~\ref{thm:fsc_decidable}), while memory-bounded policies exhibit a sharp phase transition—bounded memory gives EXPTIME, unbounded memory recovers $\Sigma_1^0$-completeness (Theorem~\ref{thm:memory_threshold}). This systematic classification (Table~\ref{tab:decidability_landscape}) clarifies which safety guarantees are algorithmically achievable.
    
    \item \textbf{Unified Capability--Risk Bound with Structural Decomposition}: We establish a unified capability--risk upper bound decomposing worst-case robust risk into three terms: empirical risk + complexity/confidence term + Lipschitz penalty $L\rho$ (Theorem~\ref{thm:unified_bound}). This framework unifies PAC-Bayes, Mutual Information stability bounds, and Wasserstein DRO into a single information-complexity functional $C(Q_S; D, P)$ (Definition~\ref{def:unified_complexity}). While Theorem~\ref{thm:unified_bound} with global Lipschitz constant $L$ is numerically vacuous in practice ($L \sim 10^{13}$ for deep networks, yielding bounds $\gg 1$), it reveals the \emph{structural} decomposition of robust risk and sets up our practical refinement.
    
    \item \textbf{Data-Dependent Practical Frontier and Lipschitz Surrogate Empirics}: We introduce a data-dependent Lipschitz constant $\bar{L} = \mathbb{E}_D[L_{\text{local}}(z)]$ (Assumption~\ref{assump:data_lipschitz}) and prove a corresponding capability--risk bound (Theorem~\ref{thm:data_dependent_bound}). On CIFAR-10/100, we systematically compare three Lipschitz estimation methods: global spectral norm product, gradient-based local averaging, and finite-difference sampling (Section~\ref{subsec:lipschitz_ablation}). Empirically, data-dependent estimators ($\bar{L} \sim 0.1$) are \emph{$10^5$ times tighter} than global bounds ($L \sim 10^{13}$), making Theorem~\ref{thm:data_dependent_bound} the first capability--risk bound with practical numerical values. We complement this with an information-theoretic minimax lower bound (Theorem~\ref{thm:minimax_lower}), proving the three-term decomposition is unavoidable.
    
    \item \textbf{SSR Pipeline Implementation and Training-Phase Safety--Performance Tradeoff}: We implement the Scope--Shield--Risk (SSR) framework's Scope and Shield layers in a complex 16${\times}$16 gridworld environment (Section~\ref{subsec:complex_ssr}). Systematic comparison reveals that SSR \emph{completely eliminates training-phase catastrophes} (baseline: 13\% catastrophe rate during training $\to$ SSR: 0\%), but in this environment the resulting policy is extremely conservative, sacrificing task completion entirely (100\% success rate $\to$ 0\%). This negative result demonstrates the \emph{real-world tradeoff} between extreme safety-first shielding and task performance, cohering with our undecidability and vacuous-bound findings: achieving absolute safety during learning requires accepting substantial capability costs. We note that our Risk Budget layer (Layer 3) is implemented as an API but not yet integrated into closed-loop deployment gating—this remains future work.
\end{enumerate}
...
\begin{remark}[Worst-Case Lipschitz Bound is Numerically Vacuous]
\label{rem:vacuous_global_bound}
Theorem~\ref{thm:unified_bound} establishes the \emph{structural} decomposition of robust risk into empirical error, complexity/confidence, and Lipschitz penalty terms. However, numerically, the bound is often \emph{completely vacuous} in practice due to the global Lipschitz constant $L$. 

For deep neural networks, estimating $L$ via layer-wise spectral norm products typically yields $L \sim 10^4$ to $10^{13}$ (see Section~\ref{subsec:lipschitz_ablation}). Even for modest distribution shift $\rho = 0.25$, this gives $L\rho \sim 10^{12}$, far exceeding any meaningful error rate. In our CIFAR-10 experiments, we observe bounds exceeding $10^{12}$ while true robust errors remain below $0.7$ (Section~\ref{subsec:cifar}).

\textbf{Value of Theorem~\ref{thm:unified_bound}:} Despite numerical vacuity, this theorem is essential for:
\begin{itemize}
    \item \emph{Structural insight}: It reveals that robust risk is fundamentally constrained by the three-term decomposition, unifying PAC-Bayes, MI, and Wasserstein perspectives.
    \item \emph{Foundation for refinement}: Theorem~\ref{thm:data_dependent_bound} (next subsection) refines this by replacing $L$ with data-dependent $\bar{L}$, yielding practical bounds orders of magnitude tighter.
\end{itemize}
For actual applications, practitioners should use Theorem~\ref{thm:data_dependent_bound}, not Theorem~\ref{thm:unified_bound} directly.
\end{remark}
...
\begin{remark}[Data-Dependent Bounds Yield Practical Capability--Risk Frontiers]
\label{rem:practical_data_dependent}
Theorem~\ref{thm:data_dependent_bound} resolves the numerical vacuity of Theorem~\ref{thm:unified_bound} by replacing global Lipschitz constant $L$ with data-dependent $\bar{L}(Q_S, D)$. In our systematic Lipschitz estimation experiments (Section~\ref{subsec:lipschitz_ablation}), we compare three estimation methods on CIFAR-10:

\begin{itemize}
\item \textbf{Global spectral norm product}: $L_{\text{global}} \sim 5 \times 10^4$ (for randomly initialized ResNet18), growing to $10^{10}$--$10^{13}$ for trained models. This renders Theorem~\ref{thm:unified_bound}'s bound completely vacuous: $L_{\text{global}} \rho \gg 1$.

\item \textbf{Gradient-based local average}: $\bar{L}_{\text{grad}} = \mathbb{E}_{z \sim D}[\|\nabla_z \ell(h, z)\|] \sim 0.11$ (Section~\ref{subsec:lipschitz_ablation}), yielding $\bar{L}_{\text{grad}} \rho \sim 0.03$ for $\rho = 0.25$. This is $\mathbf{465,765 \times}$ tighter than $L_{\text{global}}$.

\item \textbf{Finite-difference sampling}: $\bar{L}_{\text{fd}} \sim 28$, intermediate between global and gradient-based estimates.
\end{itemize}
...
\subsection{Complex Safe RL with SSR Pipeline: Training-Phase Safety vs. Task Performance Tradeoff}
\label{subsec:complex_ssr}

We implement Scope and Shield layers of the SSR framework (Section~\ref{sec:ssr}) in a complex 16${\times}$16 gridworld to investigate the real-world tradeoffs between extreme safety-first shielding and task performance. This experiment reveals a fundamental tension that coheres with our theoretical impossibility and vacuous-bound findings.

\paragraph{Environment and Task.}
\begin{itemize}
    \item \textbf{State space}: $16 \times 16 = 256$ positions. Start: (0,0). Goal: (15,15).
    \item \textbf{Hazards}: 12 point hazards + 2 rectangular hazard zones covering $\sim$15\% of grid. Entering any hazard triggers catastrophic failure (episode terminates, $R=-5$).
    \item \textbf{Obstacles}: 11 wall segments creating maze-like structure.
    \item \textbf{Rewards}: Step penalty $-0.01$, goal reward $+10$, hazard penalty $-5$.
\end{itemize}

\paragraph{SSR Implementation (Layers 1 \& 2).}
\begin{itemize}
    \item \textbf{Layer 1 (Scope Restriction)}: Within 1-step distance of any hazard zone, restrict available actions to those not entering hazards or hazard-adjacent cells.
    \item \textbf{Layer 2 (Shield)}: One-step lookahead pre-filter. If proposed action leads to hazard, replace with first available safe action from environment's action space.
    \item \textbf{Layer 3 (Risk Budget)}: Implemented as API (\texttt{check\_risk\_budget}) but \emph{not integrated} into closed-loop training or deployment gating in current experiments. This remains future work.
\end{itemize}

We train two tabular Q-learning agents (2000 episodes, $\alpha=0.1$, $\gamma=0.99$, $\epsilon$-greedy with decay): \textbf{Baseline} (no shield), \textbf{SSR} (Scope + Shield active during training and evaluation).

\paragraph{Key Result: Training-Phase vs. Evaluation-Phase Safety.}
Table~\ref{tab:complex_ssr} reports catastrophe rates and task metrics. The critical distinction is between \emph{training-phase} and \emph{final evaluation} behavior.

\begin{table}[h]
\centering
\caption{Complex GridWorld SSR Experiment: Training-Phase Safety vs. Final Task Performance. SSR \emph{completely eliminates catastrophes during training} but learns an extremely conservative policy that fails to complete the task in this environment.}
\label{tab:complex_ssr}
\begin{tabular}{@{}l|cc|cc|c@{}}
\toprule
\textbf{Configuration} & \multicolumn{2}{c|}{\textbf{Training Phase (2000 eps)}} & \multicolumn{2}{c|}{\textbf{Evaluation (100 eps)}} & \textbf{Intervention} \\
& \textbf{Catastrophes} & \textbf{Rate} & \textbf{Success} & \textbf{Catastrophe} & \textbf{Rate (eval)} \\
\midrule
Baseline (No SSR) & 265 / 2000 & 13.25\% & 100\% & 0\% & --- \\
SSR (Scope + Shield) & 0 / 2000 & \textbf{0\%} & 0\% & 0\% & 98.0 / episode \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Analysis: The Capability--Safety Tradeoff in Practice.}

\textbf{Training-Phase Safety (SSR's Success):}
\begin{itemize}
    \item Without SSR, Q-learning explores hazardous regions during training, resulting in 265 catastrophic episodes out of 2000 (13.25\% training catastrophe rate).
    \item With SSR, the shield \emph{completely blocks all catastrophic events during training} (0 out of 2000 episodes). If we view the training agent itself as a deployed system (e.g., online learning in real environments), SSR achieves its core objective: absolute safety during the learning phase.
\end{itemize}

\textbf{Evaluation-Phase Performance (SSR's Cost):}
\begin{itemize}
    \item \textbf{Baseline converges to optimal safe policy}: Despite 13\% training catastrophes, by episode 2000 the baseline has learned to avoid all hazards, achieving 100\% success rate and 0\% catastrophe rate at evaluation.
    \item \textbf{SSR learns an overly conservative policy}: The shield's aggressive restrictions (average 98 interventions per 200-step episode = 49\% intervention rate) severely limit exploration. The resulting policy converges to "stay in safe zones and do nothing," achieving 0\% success rate (never reaches goal) despite also achieving 0\% catastrophe rate.
\end{itemize}

\textbf{Return Analysis:}
\begin{itemize}
    \item Baseline evaluation: Average return $+9.71$ (reaching goal in $\sim$30 steps).
    \item SSR evaluation: Average return $-2.00$ (200 steps of $-0.01$ penalty, no goal).
\end{itemize}

\paragraph{Interpretation and Lessons.}

This negative result is \emph{informationally valuable} for several reasons:

\begin{enumerate}
    \item \textbf{Coheres with theoretical findings}: Our undecidability results (Section~\ref{sec:undecidable}) show that comprehensive safety verification is impossible for general policies. Our vacuous global Lipschitz bounds (Remark~\ref{rem:vacuous_global_bound}) demonstrate that worst-case theoretical guarantees are often meaningless numerically. This experiment shows that \emph{practical} safety mechanisms—when tuned for absolute risk aversion—can render systems useless for their intended tasks.

    \item \textbf{Clarifies SSR's design space}: SSR is \emph{not} a one-size-fits-all solution. It represents a parametric family of safety interventions. In this experiment, our shield threshold was set to "never enter hazard or hazard-adjacent cells," which proved overly conservative for this specific environment (where baseline ultimately learns to avoid hazards without such aggressive restrictions). \textbf{Optimal calibration} of shield aggressiveness requires environment-specific tuning—an open challenge.

    \item \textbf{Training-phase vs. deployment-phase objectives differ}: If the primary concern is "ensure zero catastrophes during training" (e.g., training robots in real-world environments where each failure is costly), SSR succeeded perfectly. If the objective is "learn a capable policy that remains safe at deployment," SSR failed in this environment by preventing sufficient exploration. Future work must distinguish these objectives clearly.

    \item \textbf{Demonstrates the structural tension}: This is an empirical manifestation of the capability--risk frontier. Absolute safety (training catastrophe rate $\to$ 0) came at the cost of complete task failure (success rate 100\% $\to$ 0\%). The frontier is not merely a theoretical abstraction—it constrains real system design.
\end{enumerate}

\paragraph{Limitations and Future Directions.}

\begin{itemize}
    \item \textbf{Environment-specific calibration needed}: In environments where the baseline already learns safe policies (as here), aggressive shielding is counterproductive. Future work should develop \emph{adaptive shield threshold learning} that relaxes restrictions as safety is demonstrated.
    
    \item \textbf{Risk Budget layer not yet integrated}: Our Layer 3 (Risk Budget) is implemented as an API but not wired into the experimental loop. Integrating Theorem~\ref{thm:data_dependent_bound}'s capability--risk bounds into actual deployment gating (reject policies if $\widehat{R}_S + \sqrt{C/n} + \bar{L}\rho > \varepsilon_{\text{budget}}$) is essential future work.
    
    \item \textbf{Need for environments with non-zero baseline risk}: To demonstrate SSR's positive value, future experiments should use environments where \emph{baseline training does not converge to safe policies}, creating genuine catastrophe rates at evaluation (e.g., 10--20\%). Then SSR's safety benefits could be validated without complete performance collapse.
\end{itemize}

\textbf{Conclusion of Section~\ref{subsec:complex_ssr}}: We successfully implemented SSR's Scope and Shield layers and demonstrated that they achieve absolute training-phase safety. However, in this specific environment, the resulting policy exhibits extreme conservatism that eliminates task capability. This illustrates the fundamental capability--safety tension predicted by our theory and highlights the critical need for adaptive calibration methods. Designing SSR systems that balance safety and performance remains an open engineering challenge—one that our theoretical framework (Sections~\ref{sec:undecidable}--\ref{sec:lower}) helps formalize but does not automatically solve.
...
Our theoretical results (Theorems~\ref{thm:undecidability}, \ref{thm:unified_bound}, \ref{thm:lower_bound}) jointly necessitate a layered governance approach. We \emph{propose} the \textbf{SSR (Scope--Shield--Risk) Framework} as a conceptual architecture for translating impossibility results and capability--risk tradeoffs into practical system design. Section~\ref{subsec:complex_ssr}'s experimental findings demonstrate that while SSR layers are \emph{implementable}, achieving effective balance between safety and capability requires careful environment-specific calibration—an ongoing research challenge.
...
\subsection{Relationship to Theoretical Results and Experimental Findings}

\begin{itemize}
    \item \textbf{Theorem~\ref{thm:undecidability} $\Rightarrow$ Scope + Shield}: Since verification is $\Sigma_1^0$-complete for general computable policies, we \emph{propose} (i) restricting critical control logic to decidable fragments (Scope), and (ii) runtime enforcement via shields for unrestricted components (Shield).
    
    \item \textbf{Theorem~\ref{thm:unified_bound} $\Rightarrow$ Risk Budget}: The unified capability--risk bound \emph{can in principle} provide operational deployment thresholds. However, Remark~\ref{rem:vacuous_global_bound} demonstrates that global Lipschitz versions are numerically vacuous; practical application requires data-dependent refinements (Theorem~\ref{thm:data_dependent_bound}).
    
    \item \textbf{Theorem~\ref{thm:lower_bound} $\Rightarrow$ Tradeoff Awareness}: The minimax lower bound proves the Capability--Risk tension is \emph{structural}, not a temporary algorithmic limitation. Section~\ref{subsec:complex_ssr}'s experimental result—SSR achieving 0\% training catastrophes but 0\% task success—empirically demonstrates this fundamental tradeoff. System designers must accept that absolute safety and full capability are mutually exclusive goals.
    
    \item \textbf{Open Challenge}: Section~\ref{subsec:complex_ssr} reveals that SSR layer implementation is feasible, but \emph{calibration} (shield threshold selection, risk budget setting) remains an open problem requiring environment-specific tuning and adaptive learning methods.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Systematic Empirical Validation}: Experiments across CIFAR-10, CIFAR-100, multiple training methods (ERM, PGD-AT, Spectral Norm), and Lipschitz estimation techniques confirm the frontier's universality. Data-dependent Lipschitz estimators achieve $10^5{\times}$ improvement over global bounds, validating Theorem~\ref{thm:data_dependent_bound}. Complex Safe RL experiments (16${\times}$16 gridworld with SSR Scope + Shield layers) demonstrate implementability but reveal fundamental capability--safety tensions: training-phase catastrophes eliminated (13\%$\to$0\%) at the cost of task performance collapse (100\% success $\to$ 0\%), illustrating the frontier's real-world constraints.
\end{enumerate}

These results necessitate a paradigm shift from "proving safety once" to "managing risk continuously." The SSR framework (Section~\ref{sec:ssr}) provides a conceptual architecture for this shift, though Section~\ref{subsec:complex_ssr}'s findings underscore that effective calibration of safety mechanisms remains an open engineering challenge.
