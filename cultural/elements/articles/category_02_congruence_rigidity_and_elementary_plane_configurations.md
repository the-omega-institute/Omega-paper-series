...
这一映射稍弱，但对主论文有用。Euclid 的刚性虽然看起来是静态命题，但它们在证明链中的功能更像“传播稳定性”。一旦某一步建立了某个等量或角关系，后续多步推导就可以不断携带这条结构信息，使构形始终留在允许轨道中。
...
这一类对主论文至少有四个非常具体的改写建议。

第一，把“从观测到对象”的结果写成刚性定理，而不是只写成 reconstruction theorem。Euclid 风格的表达会更强调：满足这些关系的对象只能是这一类，而不是“我们提出一个重建算法”。

第二，把 determining data 单列出来。Euclid 的强项是让读者知道哪些数据真正决定图形。你们的几何稿件也应在摘要和引言中更早地说清：到底是哪些 ratios、shells、support conditions 或 degeneracy counts 在决定对象。

第三，要区分 existence 与 rigidity。存在一个 shell、存在一个 canonical section，和它是否被唯一决定，是两个不同层次。Euclid 从不把这两件事混写。你们的主论文若能分出“构造对象”和“唯一性/刚性”两组结果，读感会更硬。

第四，要把不可见自由度如何被消灭写清楚。很多现代数学论文喜欢直接说“hence unique”，但 Euclid 风格更会交代中间每一步如何削减自由度。对机器验证和外部审稿来说，这种写法更稳。
