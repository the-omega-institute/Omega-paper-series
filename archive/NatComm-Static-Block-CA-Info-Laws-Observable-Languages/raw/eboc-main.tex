...
\textbf{Subjective time rate}: effective step-size $b=\langle\tau^\star,\tau\rangle$ only changes \textbf{observation step-frequency per unit temporal thickness}, thus normalizing by $L(W)=T$, observation entropy rate is
$$
\frac{1}{b}\,h_{\pi_\ast\mu}\!\big(\sigma_{\mathrm{time}}^{b},\alpha_R^\pi\big),
$$
...
All scripts and computational experiments described in \S8.5 (compression-entropy experiments) are available upon request. Reference implementations for SFT construction (\S7.1--7.2), decoder design (\S7.3), and SBU forcing-domain propagation (\S7.6) will be made available under open-source license (MIT or equivalent) upon publication.
