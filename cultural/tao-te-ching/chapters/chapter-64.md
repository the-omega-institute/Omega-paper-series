...
order: 64
description: "Tao Te Ching chapter 64 with source text and Omega chapter-level mapping."
...
第 64 章首先落在「无为与自然秩序」这条走廊上。它首先确认秩序不是靠外加命令维持，而是由 constraint 自发收敛出的稳定结果。 当前最强的 Omega 方向集中在 fold-operator、dynamical-systems。
...
- 本仓库原文文件：`texts/daodejing/chapter_64.txt`
